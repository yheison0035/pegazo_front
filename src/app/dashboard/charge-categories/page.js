...
} from '@/lib/api/routes/chargeCategories';
...
export default function ChargeCategoriesPage() {
  ...
  const save = async () => {
    ...
    setBusy(true);
    try {
      if (editing.id) await updateChargeCategory(editing.id, { name });
      else await createChargeCategory({ name });
      setEditing(null);
      load();
    } catch (e) {
  ...
  const doDelete = async () => {
    setBusy(true);
    try {
      await deleteChargeCategory(confirmDel.id);
      setConfirmDel(null);
      load();
    } catch (e) {
        ...
        <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
          <div>
            <h1 className="text-2xl font-semibold text-gray-800">
              Tipos de cargo a empleado
            </h1>
            <p className="text-sm text-gray-500">
              Administra los conceptos con los que cobras a tus empleados.
            </p>
          </div>
          <Button
            variant="primary"
            ...
            >
              <div className="mb-3 flex items-center justify-between">
                <h2 className="text-lg font-bold text-gray-800">
                  {editing.id ? 'Editar tipo de cargo' : 'Nuevo tipo de cargo'}
                </h2>
                <button
                  onClick={() => setEditing(null)}
              ...
              <input
                ...
                placeholder="Ej: Membresía, Préstamo, Uniforme…"
            ...
            >
              ...
              <p className="mt-2 text-sm text-gray-600">
                ...
                {confirmDel.usageCount > 0 && (
                  <>
                    {' '}
                    Tiene {confirmDel.usageCount} cargo(s) asociados; se conservan
                    en el histórico.
                  </>
                )}
              </p>
