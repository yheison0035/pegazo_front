...
} from '@/lib/api/routes/customerSegments';
...
export default function CustomerSegmentsPage() {
  ...
  const save = async () => {
    ...
    setBusy(true);
    try {
      if (editing.id) await updateCustomerSegment(editing.id, { name });
      else await createCustomerSegment({ name });
      setEditing(null);
      load();
    } catch (e) {
  ...
  const doDelete = async () => {
    setBusy(true);
    try {
      await deleteCustomerSegment(confirmDel.id);
      setConfirmDel(null);
      load();
    } catch (e) {
        ...
        <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <p className="text-sm text-gray-500">
              Etiqueta tus clientes (mayorista, VIP, frecuente…) para organizarlos.
            </p>
          </div>
          <Button
            variant="primary"
            ...
            >
              <div className="mb-3 flex items-center justify-between">
                <h2 className="text-lg font-bold text-gray-800">
                  {editing.id ? 'Editar segmento' : 'Nuevo segmento'}
                </h2>
                <button
                  onClick={() => setEditing(null)}
              ...
              <input
                ...
                placeholder="EJ: MAYORISTA, VIP, FRECUENTE…"
            ...
            >
              ...
              <p className="mt-2 text-sm text-gray-600">
                ...
                {confirmDel.usageCount > 0 && (
                  <>
                    {' '}
                    {confirmDel.usageCount} cliente(s) lo tienen; no se borran.
                  </>
                )}
              </p>
              <div className="mt-5 flex justify-end gap-2">
